...
<head>
<meta charset="UTF-8">
<title><?= isset($lancamento) ? 'Editar' : 'Novo' ?> Lançamento | Gestão Comunicação Visual</title>
    ...
    <main class="flex-fill p-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="fw-semibold mb-0">
                <?= isset($lancamento) ? 'Editar Lançamento' : 'Novo Lançamento' ?>
            </h3>
        ...
        <div class="card">
            <div class="card-body p-4">
                ...
                <form method="POST"
                      action="<?= isset($lancamento) ? APP_URL.'financeiro/atualizar' : APP_URL.'financeiro/salvar' ?>">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <label class="form-label text-white">Data de Vencimento *</label>
                            <input type="date" name="data_vencimento" class="form-control"
                                   value="<?= $lancamento['data_vencimento'] ?? date('Y-m-d') ?>" required>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status" class="form-select" required>
                                <?php
                                $statusAtual = $lancamento['status'] ?? 'pendente';
                                foreach (['pendente','pago','atrasado','cancelado'] as $s):
                                ?>
                                    <option value="<?= $s ?>" <?= $statusAtual==$s?'selected':'' ?>>
                                        <?= ucfirst($s) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="<?= APP_URL ?>financeiro" class="btn btn-outline-secondary">
                            Cancelar
                        </a>
                    </div>
